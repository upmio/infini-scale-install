...
#
# 1. POSTGRE_PWD MUST be set as environment variable, for an example:
#
#        export POSTGRE_PWD="passwords"
#
# 2. POSTGRE_USER_NAME MUST be set as environment variable, for an example:
#
#        export POSTGRE_USER_NAME="user01"
#
# 3. POSTGRE_USER_PWD MUST be set as environment variable, for an example:
#
#        export POSTGRE_USER_PWD="passwords"
#
...
#
# 6. POSTGRE_NODE_NAMES MUST be set as environment variable, for an example:
#
#        export POSTGRE_NODE_NAMES="kube-node01"
#
# 7. POSTGRE_PORT MUST be set as environment variable, for an example:
#
...
install_postgresql() {
  ...
  info "Install postgresql, It might take a long time..."
...
verify_supported() {
  ...
  if [[ -z "${POSTGRE_PWD}" ]]; then
    error "POSTGRE_PWD MUST set in environment variable."
  fi

  if [[ -z "${POSTGRE_USER_NAME}" ]]; then
    error "POSTGRE_USER_NAME MUST set in environment variable."
  fi

  if [[ -z "${POSTGRE_USER_PWD}" ]]; then
    error "POSTGRE_USER_PWD MUST set in environment variable."
  fi

  if [[ -z "${POSTGRE_STORAGECLASS_NAME}" ]]; then
    error "POSTGRE_STORAGECLASS_NAME MUST set in environment variable."
  fi

  kubectl get storageclasses "${POSTGRE_STORAGECLASS_NAME}" &>/dev/null || {
    error "storageclass resources not all ready, use kubectl to check reason"
  }

  if [[ -z "${POSTGRE_PVC_SIZE_G}" ]]; then
    error "POSTGRE_PVC_SIZE_G MUST set in environment variable."
  fi

  if [[ -z "${POSTGRE_NODE_NAMES}" ]]; then
    error "POSTGRE_NODE_NAMES MUST set in environment variable."
  fi

  if [[ -z "${POSTGRE_PORT}" ]]; then
    error "POSTGRE_PORT MUST set in environment variable."
  fi
  ...
  for node in "${db_node_array[@]}"; do
    kubectl label node "${node}" 'postgresql.standalone.node=enable' --overwrite &>/dev/null || {
      error "kubectl label node ${node} 'postgresql.standalone.node=enable' failed, use kubectl to check reason"
    }
  done
...
init_log() {
  INSTALL_LOG_PATH=/tmp/postgresql_install-$(date +'%Y-%m-%d_%H-%M-%S').log
